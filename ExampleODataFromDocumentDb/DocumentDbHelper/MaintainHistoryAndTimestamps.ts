...
function () {
    ...
    var createHistoryRecord = function (from, changeType) {
        ...
        // don't create a record on "Create" operation because it has not "changed" yet, a history record will be created if/when the first change is actually made to it
        if (changeType == operationType.create)
            return;
    ...
    if (operation == operationType.create) {
        ...
        var accepted = collection.readDocument(
            collection.getAltLink() + '/docs/' + pending.id, // can't use 'pending._self' because it might not be filled in, particularly on insert
            {},
